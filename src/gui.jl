...
if abspath(PROGRAM_FILE) == @__FILE__
    ...
    bAgent = Agent(0.3, 0.3, 0.9, 0.1)
    cAgent = Agent(0.3, 0.3, 0.9, 0.1)
    # state_holder = ControlState(Cfloat[sin(x) for x in 0:0.05:2pi], false, 1.0, bAgent)
    ...
    t_render = Renderer.render(()->ui(ctrlState, simState), width = 800, height = 600, title = "A simple UI")
